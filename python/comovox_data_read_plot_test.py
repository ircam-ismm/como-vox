...
def peak_as_script(df_acc_int, df_diff, df_rotation, tempo, rotation_threshold,
    ...
    for i in range(size - inhibition_duration - 1):   
        ...
        intensityFiltered = df_acc_int[time]
        ...
        if (time - lastBeatTime > inhibitionDuration):
            if (positiveDelta == 0):
                ...
                if (time - timeOnset < peakSearchDuration):
                    ...
                    delta_time_clipped = max(35, min(100,delta_time))
                    interval_filtered = (delta_time_clipped*(1 - 0.5) 
                                             + previous_delta*0.5)
...
#setting data
    ...
    acceleration['intFilt'] = sensors['beat']['beat.intensityFiltered']
